...
public final class Pawn extends Piece {
    ...
    public boolean isValidMove(Move move, ChessGame game) {
        ...
        if (whoseTurn == Player.WHITE) {
            if (from.getRank() == BoardRank.SECOND && move.getDeltaY() == 2) {
                return move.getDeltaX() == 0 && !game.hasPieceIn(new Square(from.getFile(), BoardRank.THIRD)) && !game.hasPieceIn(new Square(from.getFile(), BoardRank.FORTH));
            }
            ...
            if (move.getAbsDeltaX() == 1) {
                return game.hasPieceInSquareForPlayer(to, Utilities.revertPlayer(whoseTurn)) || this.canCaptureEnpassant(game, move);
            }

            if (move.getAbsDeltaX() == 0) {
                return !game.hasPieceIn(to);
            }
        } else {
            if (from.getRank() == BoardRank.SEVENTH && move.getDeltaY() == -2) {
                return move.getDeltaX() == 0 && !game.hasPieceIn(new Square(from.getFile(), BoardRank.SIXTH)) && !game.hasPieceIn(new Square(from.getFile(), BoardRank.FIFTH));
            }
    ...
    private boolean canCaptureEnpassant(ChessGame game, Move move) {
        ...
        return lastMove != null && game.getPieceAtSquare(lastMove.getToSquare()) instanceof Pawn && lastMove.getAbsDeltaY() == 2 && lastMove.getToSquare().getRank() == move.getFromSquare().getRank() && lastMove.getToSquare().getFile() == move.getToSquare().getFile();
    }
    ...
    public boolean isAttackingSquare(Square pieceSquare, Square squareUnderAttack, ChessBoard board) {
        ...
        int attackingDeltaY = this.getOwner() == Player.WHITE ? 1 : -1;
